 ...
 * @constructor
 * @param {String} request_url The OAuth request token URL.
 * @param {String} exchange_url The OAuth authorize token URL.
 ...
 */
function ChromeExOAuth(request_url, exchange_url, client_id, client_secret, oauth_scope, consumer_key, consumer_secret, opt_args) {
...
ChromeExOAuth.prototype.authorize = function(callback) {
  if (this.hasToken()) {
    callback(this.getAccessToken(), this.getRefreshToken());
  } else {
...
ChromeExOAuth.prototype.clearTokens = function() {
  delete localStorage[this.access_token + encodeURI(this.oauth_scope)];
  delete localStorage[this.refresh_token + encodeURI(this.oauth_scope)];
};
...
ChromeExOAuth.prototype.signURL = function(url, method, opt_params) {
  ...
  var secret = this.getRefreshToken();
  if (!token || !secret) {
 ...
 */
ChromeExOAuth.prototype.setAccessToken = function(token) {
  localStorage[this.access_token + encodeURI(this.oauth_scope)] = token;
};
...
ChromeExOAuth.prototype.getAccessToken = function() {
  return localStorage[this.access_token + encodeURI(this.oauth_scope)];
};
 ...
 */
ChromeExOAuth.prototype.setRefreshToken = function(refresh) {
  localStorage[this.refresh_token + encodeURI(this.oauth_scope)] = refresh;
};
 ...
 */
ChromeExOAuth.prototype.getRefreshToken = function() {
  return localStorage[this.refresh_token + encodeURI(this.oauth_scope)];
};
...
ChromeExOAuth.prototype.initOAuthFlow = function(callback) {
  if (!this.hasToken()) {
    ...
    if (params['chromeexoauthcallback'] == 'true') {
      ...
      this.requestAccessToken(oauth_token, oauth_verifier, callback);
    } else {
      var request_params = {
...
ChromeExOAuth.prototype.getRequestToken = function(callback, opt_args) {
  ...
  var signed_url = ChromeExOAuth.addURLParam(this.request_url, "redirect_uri", "http://localhost");
  console.log(url_callback);
  var signed_url = ChromeExOAuth.addURLParam(signed_url, "response_type", "code");
  var signed_url = ChromeExOAuth.addURLParam(signed_url, "client_id", this.client_id);
  var signed_url = ChromeExOAuth.addURLParam(signed_url, "scope", this.oauth_scope);
  var signed_url = ChromeExOAuth.addURLParam(signed_url, "approval_prompt", "force");
  var signed_url = ChromeExOAuth.addURLParam(signed_url, "access_type", "offline");

  var onToken = ChromeExOAuth.bind(this.onRequestToken, this, callback);
  console.log(signed_url);
  ChromeExOAuth.sendRequest("GET", signed_url, null, null, /*onToken*/ function(xhr, data) {
      console.log(data);
  });
};
...
ChromeExOAuth.prototype.onRequestToken = function(callback, xhr) {
  if (xhr.readyState == 4) {
    if (xhr.status == 200) {
      ...
      this.setRefreshToken(params['oauth_token_secret']);
      var url = ChromeExOAuth.addURLParam(this.exchange_url,
                                          "oauth_token", token);
 ...
 *         secret {String} The OAuth access token secret.
 */
ChromeExOAuth.prototype.requestAccessToken = function(oauth_token, oauth_verifier,
                                                  callback) {
  if (typeof callback !== "function") {
    throw new Error("Specified callback must be a function.");
  }
  ...
  if (bg.chromeExOAuthRequestingAccess == false) {
    ...
    var result = OAuthSimple().sign({
      ...
      signatures: {
        consumer_key : this.consumer_key,
        shared_secret : this.consumer_secret,
        oauth_secret : this.getRefreshToken(this.oauth_scope)
      }
    });
